...
export default function MemberConnectionsPage() {
  ...
  const addBookmark = () => { if (!graphRef.current) return; const bm = graphRef.current.saveBookmark(`Закладка ${bookmarks.length + 1}`); setBookmarks([...bookmarks, bm]); saveBookmarks([...bookmarks, bm]); setShowBookmarks(false); };
  const restoreBookmark = (bm: Bookmark) => { graphRef.current?.restoreBookmark(bm); setShowBookmarks(false); };
  ...
  const rightActions = (
    <>
      <div className="relative">
        <button onClick={() => setShowBookmarks((p) => !p)} className="p-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-500 hover:text-amber-400 transition-colors" title="Закладки"><BookmarkIcon className="w-4 h-4" /></button>
        {showBookmarks && (
          <div className="absolute top-full right-0 mt-2 w-52 rounded-xl bg-[#131b2e] border border-white/10 shadow-2xl z-50 overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 border-b border-white/5">
              <span className="text-[11px] text-gray-400">Закладки</span>
              <button onClick={addBookmark} className="text-emerald-400 hover:text-emerald-300"><BookmarkPlus className="w-3.5 h-3.5" /></button>
            </div>
            {bookmarks.length === 0 && <p className="text-[10px] text-gray-600 px-3 py-3 text-center">Нет закладок</p>}
            {bookmarks.map((bm) => (
      ...
      <button onClick={() => { (graphRef.current as any)?.resetSun?.(); handleReset(); }} className="p-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-500 hover:text-amber-400 transition-colors" title="Исходное состояние"><RotateCcw className="w-4 h-4" /></button>
